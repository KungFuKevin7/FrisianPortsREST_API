...
namespace FrisianPortsREST_API.Repositories
{
    public class PeriodRepository
    {
        ...
        public async Task<List<int>> GetAllYearsOfPort(int portId)
        {
        ...
        /// <param name="portId">Id of desired port</param>
        /// <returns>List of all availible years for port</returns>
        public async Task<List<int>> GetAllYearsOfProvince(int portId)
        {
            using (var connection = DBConnection.GetConnection())
            {
                connection.Open();
                ...
                const string query = $@"SELECT DISTINCT YEAR(T.DEPARTURE_DATE)
                                        ...
                                        INNER JOIN ROUTE R
                                        ON CT.ROUTE_ID = R.ROUTE_ID
                                        INNER JOIN PORT P ON R.ARRIVAL_PORT_ID = P.PORT_ID OR DEPARTURE_PORT_ID = P.PORT_ID
        ...
        public async Task<List<YearlyTransportDTO>> getYearlyReportPort(int portId)
        {
            List<YearlyTransportDTO> yearlyTransportDTOs
                   = new List<YearlyTransportDTO>();

            var years = await GetAllYearsOfPort(portId);
        ...
        /// Get import and export data per port on a yearly basis
        /// </summary>
        /// <param name="provinceId">Id of desired port</param>
        /// <returns>List of Import and Export per year from port</returns>
        public async Task<List<YearlyTransportDTO>> getYearlyReportProvince(int provinceId)
        {
            List<YearlyTransportDTO> yearlyTransportDTOs
                   = new List<YearlyTransportDTO>();

            var years = await GetAllYearsOfPort(provinceId);
